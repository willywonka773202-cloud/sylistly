...
import { CLIENT_CATALOG_PRODUCTS, isFreshVerifiedClientCatalogProduct } from '@/lib/client-catalog';
...
function hydrate(record: BakedLookRecord): AiLook | null {
  ...
  // Every product must clear the same exact-PDP/trust/stock/known-health gate as
  // the primary feed. A premium badge and public share URL can never excuse a
  // partially shoppable look; old baked rows are withheld until they are
  // re-baked from the current catalog.
  if (!Object.values(products).every((product) => isFreshVerifiedClientCatalogProduct(product))) return null;
  return {
    id: record.id,
